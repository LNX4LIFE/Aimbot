...
struct Aimbot {
    ...
    void RenderUI() {
        if (ImGui::BeginTabItem("Aim", nullptr, ImGuiTabItemFlags_NoCloseWithMiddleMouseButton | ImGuiTabItemFlags_NoReorder)) {
            ...
            if (ImGui::BeginChild("Aimbot Main", ImVec2(TabSize.x - TabSize.x , (TabSize.y - TabSize.y) + 270), true)) {
                ...
                if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
                    ImGui::SetTooltip("Will enable using mouse on xap/grinder and mem recoil with memory aim(better recoil control than mouse)however Legitbot might be worse");
                ...
                if (AimbotMode == 0 || AimbotMode == 2 && !AdvancedAim) {
                    ...
                    if (!ClosestHitbox) {
                        ...
                        if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
                        ImGui::SetTooltip("Which bone the Legitbot will aim at.");
                ...
                if (AimbotMode == 0 && !AdvancedAim) {
                    ...
                    if (ImGui::CollapsingHeader("Smoothness Settings", nullptr)) {
                        ...
                        if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
                            ImGui::SetTooltip("Delay time for the Legitbot smoothing.\n");
                    }
                            ...
                            if (P2020) {
                                if (ImGui::CollapsingHeader("P2020", nullptr)) {
                                    ...
                                    if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
                                        ...
                                        if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
                                            ImGui::SetTooltip("If the Legitbot is close enough then the Legitbot will stop trying to get any closer.\n If you have very low smoothing then you might want to up this to prevent 'shaking'.");
    ...
    void Update() {
        ...
        if(Modules::Home::IsMenuOpened) return; //Dont Legitbot whilst menu is open
    ...
    void StartAiming() {
        ...
        if (AdvancedAim) {
            SmoothAngle(CurrentTarget, DesiredAngles);
        Vector2D LegitbotDelta = Vector2D(CalculatePitchIncrement(DesiredAngles), CalculateYawIncrement(DesiredAngles)).Multiply(AdvancedSpeed);


        int totalYawIncrementInt = RoundHalfEven(AL1AF0(LegitbotDelta.x));
        int totalPitchIncrementInt = RoundHalfEven(AL1AF0(LegitbotDelta.y * -1));
        ...
        if(AimbotMode == 0) {
            ...
            Vector2D LegitbotD = Vector2D(CalculatePitchIncrement(DesiredAngles), CalculateYawIncrement(DesiredAngles)).Multiply(Speed);

            int totalYawIncrementInt = RoundHalfEven(AL1AF0(LegitbotD.x));
            int totalPitchIncrementInt = RoundHalfEven(AL1AF0(LegitbotD.y * -1));
    ...
    //Aimbot calcs
    const FloatVector2D LegitbotDelta = CurrentTarget->LegitbotDesiredAnglesIncrement
        .multiply(100)
        .divide(TOTAL_SMOOTH);
    const double aimYawIncrement = LegitbotDelta.y * -1;
    const double aimPitchIncrement = LegitbotDelta.x;
    //Combine
    ...
    if (fabs(CurrentTarget->LegitbotDesiredAnglesIncrement.x) < AdvancedDeadzone) totalPitchIncrementInt = 0;
    if (fabs(CurrentTarget->LegitbotDesiredAnglesIncrement.y) < AdvancedDeadzone) totalYawIncrementInt = 0;
    if (totalPitchIncrementInt == 0 && totalYawIncrementInt == 0) return;
    //move mouse
    ...
    bool active() {
        bool LegitbotIsOn = AimbotEnabled;
    ...
    void assignTarget() {
        for (int i = 0;i < Players->size();i++) {
            ...
            if (TeamCheck) {
                ...
                if (fabs(p->LegitbotDesiredAnglesIncrement.x) > FOV1) continue;
                if (fabs(p->LegitbotDesiredAnglesIncrement.y) > FOV1) continue;
                if (CurrentTarget == nullptr || p->LegitbotScore > CurrentTarget->LegitbotScore) {
                CurrentTarget = p;
                CurrentTarget->LegitbotLocked = true;
                }
        }
            if (!TeamCheck) {
                if (!p->IsCombatReady())continue;
    ...
    void releaseTarget2() {
        if (CurrentTarget != nullptr && CurrentTarget->IsValid())
            CurrentTarget->LegitbotLocked = false;
        CurrentTarget = nullptr;
    }
